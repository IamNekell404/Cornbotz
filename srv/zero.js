...
var num = "6281224413305@s.whatsapp.net"
var ownerNumber = ["6289695073357@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
...
success: 'Done...Semoga Harimu Haha Hihi',
...
status: 1,
surface : 1,
message: `Facebook : Nekell Ganss`, 
orderTitle: `Bissmilah Jadi Seleb`,
thumbnail: fakethumb,
...
「 INFO BOT 」
🎃Creator Bot : '©Created By NekellGanss.'
🌐 Prefix :  ⌜  ${prefix}  ⌟
🍬 Name Bot : ${NameBot}
...
var btngrass = {
contentText: `${anu}`,
footerText: 'Created By NekellGanteng',
buttons: punten,
headerType: 1
}
...
costum(buffer, sticker, Verived, `Jangan Lupa Bahagia Hari Ini:)`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `Jangan Lupa Follow FB : Nekell Ganss`)
fs.unlinkSync(ran)
})
break
...
zero.sendMessage(`6289695073357@s.whatsapp.net`, options, text, { quoted: ftrol })
reply('Masalah Telah Di Laporkan Ke Owner BOT, Mohon Tunggu Untuk Proses Perbaikan')
break
case 'youtube':
teks =
`Nih Youtube Owner Ku Jangan Lupa Di Subscribe Ya https://youtube.com/NekellGanss`
zero.sendMessage(from, teks, text, {quoted: ftrol})
break
case 'instagram':
teks =
`Nih Instagram Owner Ku Jangan Lupa Di Follow Ya https://instagram.com/nekellgatau`
zero.sendMessage(from, teks, text, {quoted: ftrol})
break
case 'tiktok':
teks =
`Nih Tiktok Owner Ku Jangan Lupa Di Follow Ya https://tiktok.com/@abcdefghijkell`
zero.sendMessage(from, teks, text, {quoted: ftrol})
break
case 'sourcecode':
case 'script':
...
*┃◗ Thank To Ortu*
*┃◗ Thank To NekellGanteng (Creator)*
...
case 'hacked':
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   ...
                   await sleep(1000)
                zero.groupUpdateSubject(from, `TERKHENTOD BY ${body.slice(8)}`)
                await sleep(1000)
...
var ZeroYT7 = {
           ...
           quotedMessage: {
           imageMessage: {
           caption: `©Created By NekellGanteng`,
           ...
           zero.sendMessage(from, rules, MessageType.text, NekellGanteng)
           break 
case 'tag':
...
case 'codwarzone':
case 'valorantbanner':
if (args.length == 0) return reply(`Teks Nya Mana ?\nContoh : ${prefix + command} NekellGanss`)
bo = args.join(" ")
reply(mess.wait)
anu = await getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey1}&text=${bo}`)
zero.sendMessage(from, anu, image, {quoted: ftrol, caption: 'Done Jangan Lupa Bahagia (CornBotz)' })
break
case 'hartatahta':
case 'naruto':
case 'bneon':
...
case 'tlight':
case 'nulis':
if (args.length == 0) return reply(`Teks Nya Mana\nContoh : ${prefix + command} NekellGanteng`)
bo = args.join(" ")
reply(mess.wait)
res = await getBuffer(`https://api.zeks.me/api/${command}?apikey=${apikey4}&text=${bo}`)
zero.sendMessage(from, res, image, {quoted: ftrol, caption: 'Done Jangan Lupa Bahagia (CornBotz)' })
break
case 'wolflogo':
case 'pubglogo':
case 'snowwrite':
...
zero.sendMessage(from, wll, image, {quoted: ftrol, caption: 'Done Jangan Lupa Bahagia (CornBotz)' })
break
case 'blackpink':
case 'neon':
case 'greenneon':
...
zero.sendMessage(from, blck, image, {quoted: ftrol, caption: 'Done Jangan Lupa Bahagia (CornBotz)' })
break
case 'ssweb':
if (args.length == 0) return reply(`Teks Nya Mana ?\nContoh: ${prefix + command} Nekell Gantenk`)
ini_url = args.join(" ")
reply(mess.wait)
getBuffer(`https://ziy.herokuapp.com/api/ssweb?apikey=xZiyy&url=${ini_url}`).then((hasil) => {
zero.sendMessage(from, hasil, image, { thumbnail: Buffer.alloc(0), caption: `Done Jangan Lupa Bahagia (CornBotz)`, quoted : ftrol})
})
break
case 'asupan':
...
zero.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Nih Jangan Lupa Bahagia (CornBotz)'})
break
case 'asupancecan':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=${apikey5}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
zero.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Nih Jangan Lupa Bahagia (CornBotz)'})
break
case 'asupanhijaber':
